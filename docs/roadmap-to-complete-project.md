...
| **R2** | **Client surface for “play”** | **Done (v3):** Python **`observation_hud.py`** + Rust **`aetherforge_window`** (winit + wgpu + in-proc sim; title-bar HUD — [**`platform-headed-roadmap.md`**](platform-headed-roadmap.md) P1–P3). Next: **P4** input → intent or external Godot shell. |
| **R3** | **Runtime / embedding decision** | **Done (v1):** [**ADR 0004**](adr/0004-runtime-embedding.md) — Godot 4.x default shell for full game client; Unity / web / in-repo Rust platform as alternatives; **`aetherforge_serve`** HTTP as canonical contract. |
