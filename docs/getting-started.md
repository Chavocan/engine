...
Opens a **960×540** window: **wgpu** clear + in-process **farm-stub** sim. **Keyboard** sends intents (**P** or **1** plant, **D** or **2** day, **H** or **3** harvest, **Space** noop) — same `kind` strings as HTTP. The **title bar** and an **in-window** bitmap text overlay show state + a short keymap. Optional **`AETHERFORGE_WINDOW_AUTO_DEMO=1`** replays the scripted 5-step loop every frame (no keys). Requires a GPU driver; close the window or set a timeout:
